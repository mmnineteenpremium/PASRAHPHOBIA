...
function UISystem:_applyRoomBrowserSizing(profile, viewportSize, topLeftInset, bottomRightInset)
	...
	if isCompact then
		...
		local roomListHeight = actionY - roomListY - 8
		if roomListHeight < (profile.isMobile and 153 or 140) then
			local deficit = (profile.isMobile and 153 or 140) - roomListHeight
			previewHeight = math.max(profile.isMobile and 212 or 224, previewHeight - deficit)
			roomListY = contentTop + previewHeight + 12
			roomListHeight = math.max(profile.isMobile and 153 or 140, actionY - roomListY - 8)
		end
		...
		setOffsetBounds(joinPassword, headerPadding, actionY - (profile.isMobile and 43 or 42), previewWidth, extraCompactMobile and 34 or (profile.isMobile and 38 or 34))
		setOffsetBounds(queueButton, headerPadding, actionY, previewWidth, queueHeight)
		setOffsetBounds(quickClassicButton, headerPadding, stackFirstRowY, math.floor((previewWidth - 4) * 0.5), stackButtonHeight)
		setOffsetBounds(quickRankedButton, headerPadding + math.floor((previewWidth - 4) * 0.5) + 4, stackFirstRowY, math.floor((previewWidth - 4) * 0.5), stackButtonHeight)
		setOffsetBounds(refreshButton, headerPadding, stackSecondRowY, math.floor((previewWidth - 4) * 0.5), stackButtonHeight)
		setOffsetBounds(createRoomButton, headerPadding + math.floor((previewWidth - 4) * 0.5) + 4, stackSecondRowY, math.floor((previewWidth - 4) * 0.5), stackButtonHeight)

		local previewMapHeight = math.clamp(math.floor(previewHeight * (profile.isMobile and 0.43 or 0.45)), profile.isMobile and 120 or 112, profile.isMobile and 141 or 136)
		...
		setOffsetBounds(roomPreviewPlayersList, 12, (profile.isMobile and 72 or 66) + previewMapHeight + 27, previewWidth - 24, previewHeight - ((profile.isMobile and 72 or 66) + previewMapHeight + 37))
		if previewMapTitle then
			setOffsetBounds(previewMapTitle, 16, 8, previewWidth - 24, 14)
			previewMapTitle.TextSize = profile.isMobile and 11 or 10
		end
		if previewMapMood then
			local moodWidth = extraCompactMobile and 113 or 144
		...
		if previewMapFooter then
			setOffsetBounds(previewMapFooter, 16, 27, previewWidth - 48, extraCompactMobile and 28 or 44)
			previewMapFooter.TextSize = extraCompactMobile and 12 or (profile.isMobile and 14 or 13)
			previewMapFooter.TextTruncate = extraCompactMobile and Enum.TextTruncate.AtEnd or Enum.TextTruncate.None
		end
		if previewMapStats then
			setOffsetBounds(previewMapStats, 16, previewMapHeight - 21, previewWidth - 48, 16)
