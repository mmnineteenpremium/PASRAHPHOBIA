...
function UISystem:_applyRoomBrowserSizing(profile, viewportSize, topLeftInset, bottomRightInset)
	...
	if isCompact then
		...
		local roomListHeight = actionY - roomListY - 9
		if roomListHeight < (profile.isMobile and 155 or 140) then
			local deficit = (profile.isMobile and 155 or 140) - roomListHeight
			previewHeight = math.max(profile.isMobile and 212 or 224, previewHeight - deficit)
			roomListY = contentTop + previewHeight + 12
			roomListHeight = math.max(profile.isMobile and 155 or 140, actionY - roomListY - 9)
		end
		...
		setOffsetBounds(joinPassword, headerPadding, actionY - (profile.isMobile and 45 or 42), previewWidth, extraCompactMobile and 34 or (profile.isMobile and 38 or 34))
		setOffsetBounds(queueButton, headerPadding, actionY, previewWidth, queueHeight)
		setOffsetBounds(quickClassicButton, headerPadding, stackFirstRowY, math.floor((previewWidth - 5) * 0.5), stackButtonHeight)
		setOffsetBounds(quickRankedButton, headerPadding + math.floor((previewWidth - 5) * 0.5) + 5, stackFirstRowY, math.floor((previewWidth - 5) * 0.5), stackButtonHeight)
		setOffsetBounds(refreshButton, headerPadding, stackSecondRowY, math.floor((previewWidth - 5) * 0.5), stackButtonHeight)
		setOffsetBounds(createRoomButton, headerPadding + math.floor((previewWidth - 5) * 0.5) + 5, stackSecondRowY, math.floor((previewWidth - 5) * 0.5), stackButtonHeight)
		...
		if previewMapTitle then
			setOffsetBounds(previewMapTitle, 16, 8, previewWidth - 26, 14)
			previewMapTitle.TextSize = profile.isMobile and 11 or 10
		end
		if previewMapMood then
			local moodWidth = extraCompactMobile and 114 or 144
		...
		if previewMapFooter then
			setOffsetBounds(previewMapFooter, 16, 28, previewWidth - 48, extraCompactMobile and 29 or 44)
