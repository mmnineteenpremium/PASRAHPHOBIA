...
function Service:HandleEvent(eventName, payload)
    ...
    if eventName == "MatchStarted" then
        self._state:Set("spectators", {})
    elseif eventName == "PlayerDied" then
        local userId = toUserId(payload and (payload.player or payload.userId))
        if userId then
            local spectators = self._state:Get("spectators") or {}
            spectators[userId] = true
            self._state:Set("spectators", spectators)
            self:_publish("SpectatorModeStarted", { userId = userId, player = payload.player, matchId = self._state:Get("activeMatchId") })
        end
