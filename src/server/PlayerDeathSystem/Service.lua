...
function Service:HandleEvent(eventName, payload)
    ...
    elseif eventName == "PlayerDied" then
        ...
        if userId then
            ...
            table.insert(deathLog, { userId = userId, reason = payload and payload.reason or "unknown", at = os.clock() })
            self._state:Set("deathLog", deathLog)
            local drops = self._state:Get("droppedInventory") or {}
            drops[userId] = payload and payload.inventoryDrop or { "Flashlight" }
            self._state:Set("droppedInventory", drops)
            self:_publish("PlayerDeathProcessed", { userId = userId, player = payload.player, reason = payload and payload.reason, droppedItems = drops[userId], matchId = self._state:Get("activeMatchId") })
        end
    end
end
return Service
