...
function Service:HandleEvent(eventName, payload)
    ...
    if eventName == "HuntStarted" then
        self._state:Set("escapingPlayers", {})
    elseif eventName == "GhostInteraction" then
        ...
        if userId and action == "EscapedGhost" then
            local escaping = self._state:Get("escapingPlayers") or {}
            escaping[userId] = true
            self._state:Set("escapingPlayers", escaping)
            self:_publish("PlayerEscapedHunt", { userId = userId, player = payload.player, matchId = self._state:Get("activeMatchId") })
        end
    elseif eventName == "HuntEnded" then
        local extracted = self._state:Get("extractedPlayers") or {}
        for userId, escaped in pairs(self._state:Get("escapingPlayers") or {}) do
            if escaped == true then extracted[userId] = true end
        end
        self._state:Set("extractedPlayers", extracted)
