...
function TelemetrySystem.new(deps)
	...
	self._deps = deps or {}
	self._created = false
	self.State = State.new(self._deps.TelemetryState)
...
function TelemetrySystem:Initialize()
	...
	if registry and not hasService(registry, "TelemetrySystem") then
		registerService(registry, "TelemetrySystem", self)
	end
end
