...
- Owner-visible continuation smoke terbaru:
  ...
  - Follow-up source patch: `GhostSystem.Service` sekarang punya texture-donor lane internal yang tidak mengganti rig animasi. Runtime tetap spawn dari normalized skinned `inventoryModelAssetId`, lalu hanya jika mesh/surface belum punya texture usable, service mencoba load static/team texture donor asset dan menyalin `SurfaceAppearance`/`TextureID`/warna ke mesh runtime. Source `sourcemap` dan Rojo build pass; live Studio visual retest masih perlu saat Roblox Studio MCP kembali tersedia.
...
- `2026-05-17` local owner-visible smoke in `brian-second-final`:
  ...
  - Source-lock follow-up for investigation-tool visuals: `ToolVisualAssetSystem` and the second-account `ToolVisualConfig` are now treated as branch source, not loose local drift. `StudioE2EControlSystem` is excluded from normal `SystemRegistry` loading so Studio test controls remain opt-in through the bootstrap attribute instead of becoming a default runtime owner.
  ...
  - Rojo `TexturePack` dependency permission noise was isolated to four source `SurfaceAppearance.TexturePack` properties (`Salib`, `Dupa`, `Garam`, `Pocong`). Those `TexturePack` fields were removed while retaining direct PBR map asset IDs (`ColorMapContent`, `NormalMapContent`, `RoughnessMapContent`, `MetalnessMapContent`) so runtime visuals do not depend on package texture upload ownership.
  - Follow-up source binary cleanup removed embedded `SurfaceAppearance.TexturePack` from 32 `.rbxm` assets under `Workspace`, `ReplicatedStorage`, and `ServerStorage` maps/ghost/tool templates. Rojo build and sourcemap pass after cleanup; remaining visual polish is owner-approved texture/material replacement where direct PBR maps are not already present.
