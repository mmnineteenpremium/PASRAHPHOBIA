...
- `2026-05-17` local owner-visible smoke in `brian-second-final`:
  ...
  - Non-strict release preflight from `brian-second-final` passed without writing `PASRAHPHOBIA.rbxlx`: Rojo build ok to `C:\tmp\pasrah-brian-release-preflight.rbxlx`, marketplace mapping clean (`safeItemsMissingMarketplaceId=0`, `safeItemsStillDisabled=0`, `holdItemsAccidentallyEnabled=0`, `unclassifiedItems=0`). Remaining scripted blocker is the intended manual/real 2-client smoke.
